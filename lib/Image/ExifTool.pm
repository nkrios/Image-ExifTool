...
%EXPORT_TAGS = (
    Public => [ qw(
        ...
        SetNewValue SetNewValuesFromFile GetNewValues SetNewGroups GetNewGroups
        GetTagID GetDescription GetGroup GetGroups BuildCompositeTags
        GetTagName GetShortcuts GetAllTags GetWritableTags GetAllGroups
        GetFileType
    )],
    DataAccess => [qw(
...
sub new
{
    ...
    # set default write priorities (priority keys all lower case!)
    $self->{WRITE_GROUPS} = [ 'EXIF', 'GPS', 'IPTC', 'XMP', 'MakerNotes' ];
    $self->{WRITE_PRIORITY} = { exif => 5, gps => 4, iptc => 3, xmp => 2,
                                makernotes => 1, file => 10, composite => 10 };
