...
#
                    ...
                    if ($isOverwriting) {
                        ...
                        unless (length $newVal) {
                            $exifTool->Error("Can't write zero length $$newInfo{Name} in $tagTablePtr->{GROUPS}->{1} IFD");
                            return undef;
                        }
...
#
    ...
    if (not $dirInfo->{Fixup}) {
        if ($dirInfo->{NewDataPos}) {
        ...
        if ($previewInfo) {
                ...
                $previewInfo->{IsValue})
            {
                ...
                my $newPos = length($newData) + ($dirInfo->{NewDataPos} || 0);
