...
form, but tEXt chunks are written by default when creating new tags.
Compressed zTXt chunks are written only if Compress::Zlib is available, and
only for profile information or when the -z (Compress) option is specified.
