...
B<SetNewValue>.  The necessary segments and/or directories are created in
the destination file as required to store the specified information.
...
PrintConv values are the values returned by B<ImageInfo> and B<GetInfo> in
the tag/value hash.
...
written to hightest priority group as specified by SetNewGroups().  Any
...
before B<ImageInfo> returns.  However if you enable the Composite option,
B<BuildCompositeTags> is called from within B<ImageInfo> before the file is
closed.
