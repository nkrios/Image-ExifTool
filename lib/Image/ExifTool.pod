...
Character set for converting special characters.  Valid values are:
...
Flag to cause MakerNotes and other writable subdirectories (such as PrintIM)
to be extracted as a data block.  Normally when the MakerNotes are extracted
they are rebuilt to include data outside the boundaries of the original
maker note data block, but a value of 2 disables this feature.  Possible
values are:
...
1) Source file name, file reference, scalar reference, or undefined to
...
GraphConv, HP, HTML, HTML-dc, HTML-ncc, HTML-prod, HTML-vw96, HTTP-equiv,
